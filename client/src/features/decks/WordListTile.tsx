...
export const DeckTile = SortableElement(DeckTileComponent);

function DeckTileComponent({ path, wordlist, onDeleteDeck }: DeckTileProps) {
  return (
    <NavLink
      key={wordlist.name}
    ...
    >
      ...
      <IconButton
        onClick={e => {
          e.preventDefault();
          wordlist.id && onDeleteDeck(wordlist)
        }}
      >
        <DeleteIcon />
      </IconButton>
